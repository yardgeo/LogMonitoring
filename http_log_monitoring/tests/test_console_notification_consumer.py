...
class TestConsoleNotificationConsumer(IsolatedAsyncioTestCase):
    ...
    async def _assert_log_output(self,
                                 notification: NotificationDto,
                                 expected_output: str):
        # mock all logs with ctx
        with self.assertLogs() as ctx:
            await self._console_notification_consumer.consume_notification(
                notification
            )
    ...
    async def test_consume_notification_info_level(self):
        msg = "test"
        expected_output = "INFO:root:test"
        notification = NotificationDto(message=msg,
                                       level=NotificationLevelDto.INFO,
                                       type="any")
        await self._assert_log_output(notification=notification,
                                      expected_output=expected_output)

    async def test_consume_notification_critical_level(self):
        msg = "test"
        expected_output = "CRITICAL:root:test"
        notification = NotificationDto(message=msg,
